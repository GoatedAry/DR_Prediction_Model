...
class DROrdinalRegressor(nn.Module):
    def __init__(self, dropout: float = 0.4, freeze_until_layer: str = "layer3", pretrained: bool = False):
        super().__init__()
        # pretrained=False by default: we always load a full trained
        # checkpoint (best_model.pt) right after construction, which
        # overwrites every weight anyway - so there's no need to download
        # ImageNet weights from the internet just to discard them.
